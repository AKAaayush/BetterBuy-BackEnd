...
class AuthController {
  login(req, res) {
    user.findOne(
      { email: req.body.email, type: "Account" },
      function (err, user) {
        if (err) {
  ...
  sendVerificationCode(req, res) {
    ...
    client.verify.services
      .create({ friendlyName: "MeroPasal", codeLength: "4" })
      .then(function (service) {
        console.log(service.sid);
        client.verify
          .services(service.sid)
          .verifications.create({ to: "+977" + req.body.phone, channel: "sms" })
  ...
  verifyCode(req, res) {
    ...
    client.verify
      .services(req.body.sid)
      .verificationChecks.create({
        to: "+977" + req.body.phone,
        code: req.body.code,
      })
